...
* **M14.4: Capability-Governed Harness Bindings & Interactive REPL Commands**
   - Implement native VM bindings in ``src/kernel/harness_bindings.zig``: ``sys_cas_put(data)`` (returns 64-character hex BLAKE3 string), ``sys_cas_get(hash)`` (returns stored chunk content string), ``sys_actor_persist(id)`` (snapshots active actor source code and manifest to CAS), and ``sys_actor_spawn_cas(hash)`` (retrieves chunk, compiles, and spawns isolated child actor).
...
* **M14.5: End-to-End Headless Verification & Reboot Persistence**
   ...
   - Implement automated two-stage reboot regression: Stage 1 boots QEMU, stores actor script into CAS via REPL, records emitted hash, and halts VM cleanly; Stage 2 boots fresh QEMU instance with the same persistent disk image, invokes ``spawn_cas <hash>``, and asserts identical execution on hardware without host or network assistance.
