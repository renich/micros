...
- **Interactive Shell Ergonomics & Resident AI Protocol Hardening**:
  - **First-Class Framebuffer Terminal for MicroShell**: Updated ``lib/macros/msh.mx`` (App 0) to render interactive shell sessions directly to the 1280x800 UEFI GOP framebuffer (``tty0``) with header banner, prompt styling, character-by-character backspace erasing, and auto-scrolling, while maintaining simultaneous serial console mirroring (``ttyS0``). Restores the terminal display seamlessly upon return from App 1 (``harness``) or ``installer``.
