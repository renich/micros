    ...
    pub fn nextToken(self: *Lexer) Token {
        while (self.position < self.source.len) {
            ...
            switch (c) {
                ...
                '+' => {
                    self.advance();
                    return Token{ .token_type = .plus, .lexeme = "+" };
                },
                '-' => {
                    self.advance();
                    return Token{ .token_type = .minus, .lexeme = "-" };
                },
                '=' => {
                    self.advance();
                    if (self.peek() == '=') {
                        self.advance();
                        return Token{ .token_type = .equal_equal, .lexeme = "==" };
                    }
                    ...
                    if (isAlpha(c)) {
                        ...
                        self.advance();
                        return Token{ .token_type = .invalid, .lexeme = self.source[self.position - 1 .. self.position] };
                    }
                },
            }
        }
